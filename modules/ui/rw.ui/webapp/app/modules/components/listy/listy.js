...
export default class Listy extends React.Component {
	constructor(props) {
 	...
 	// TODO: Put in utils
 	// Following three functions are copied and/or derived from lodash
 	...
 	createList(data, iter=0) {
 		...
 		var listNode = null;
 		var self = this;
 		if (Utils.isArrayLike(data) && Utils.isObjectLike(data)) {
 			var children = [];
			data.forEach(function(element, index, array) {
				if (Utils.isCollection(element)) {
 		...
 		else if (Utils.isObjectLike(data)) {
 			var children = [];
 			for (var key in data) {
 				if (Utils.isCollection(data[key])) {
 					children.push(
