			...
			<div className={ClassNames(rsp.className, {'-is-factory': rsp.isFactory, '-is-edit-paths-mode': isEditPathsMode})}>
				<LayoutRow primaryActionColumn={toggleSelectionOrCreateNewPath} secondaryActionColumn={null}>
					...
					<div className="connection-points">
						{rsp.connectionPoints.map(mapConnectionPoint.bind(null, context.stylePrimary, true))}
						...
						/> : null}
						{!hasServiceFunctionVNFDs && !rsp.isFactory ? <small className="hint">A VNFD with the chain SF is required to build Record Service Paths.</small> : null}
