...
rw_status_t CliManifest::show()
{
  ...
  if (rw_cli.schema_mgr_->schema_table_.size()) {
...
rw_status_t CliManifest::add_cli_print_hook(const char *path, const char *hook)
{
  ...
  RwCliParser *tmp_parser = new RwCliParser(*(rw_cli.schema_mgr_->model_), rw_cli,
                                           ...
                                           const std::set<std::string> & module_names)
{
  ...
  if (std::find(mfiles.begin(), mfiles.end(), name) != mfiles.end()) {
    std::cout << "CLI Manifest file already loaded " << name << std::endl;
    return RW_STATUS_SUCCESS;
  }
  ...
  messaging_hook_(nullptr)
{
  ...
  if (schema_listing) {
    schema_listing_.assign(schema_listing);
    load_schema_modules(schema_listing_.c_str());
  }
}
...
RwCLI::~RwCLI()
{
  if (parent_rw_cli) {
    ...
    if (parent_rw_cli->editline == editline) {
      editline = nullptr;
    }
...
bool RwCLI::show_cli_history(const ParseLineResult& r) {
  ...
  if (editline == NULL) {
    // ZSH stores the history for RW.CLI editline history will not be used
    if (history_hook_) {
      history_hook_();
    }
    return true;
  ...
  History *h = editline->hist;
  HistEvent ev, curr;

  RW_ZERO_VARIABLE (&ev);
  RW_ZERO_VARIABLE (&curr);
  // if the number of elements is 0, return
  if (0 > history(h, &ev, H_GETSIZE)) {
    return true;
  }

  // save off the current node
  if (0 > history (h, &curr, H_CURR)) {
    return true;
  }
...
bool RwCLI::clear_cli_history(const ParseLineResult& r) {
  ...
  UNUSED (r);
  RW_ZERO_VARIABLE (&ev);
  RW_ASSERT (0 <= history (editline->hist, &ev, H_CLEAR));
  return true;
}
                                       ...
                                       NetconfEditConfigOperations ec)
{
  ...
  if (messaging_hook_) {
    return messaging_hook_(&req, rsp);
  }
...
void RwCLI::add_builtin_cmds()
{
  ...
  xml_keyword->set_cli_print_hook(".:pretty_print_xml_file");
...
static void
rwcli_io_callback(rwsched_CFSocketRef s,
		  CFSocketCallBackType type,
		  CFDataRef address,
		  const void *data,
		  void *info)
{
  // This is the CFSocket callback function that was registered for stdin
  // Pass control into a C++ riftcli class function
  UNUSED (s);
  UNUSED (type);
  UNUSED (address);
  UNUSED (data);

  RwCLI *rw_cli = (RwCLI *) info;
  rw_cli->editline->read_ready();
}

void
RwCLI::readline_loop(void)
{
  ...
  cfsocket = rwsched_tasklet_CFSocketCreateWithNative(instance->rwtasklet_info->rwsched_tasklet_info,
  ...
  rwsched_tasklet_CFSocketSetSocketFlags(instance->rwtasklet_info->rwsched_tasklet_info, cfsocket, cf_option_flags);
  cfsource = rwsched_tasklet_CFSocketCreateRunLoopSource(instance->rwtasklet_info->rwsched_tasklet_info,
						 kCFAllocatorSystemDefault,
						 cfsocket,
						 0);
  RW_CF_TYPE_VALIDATE(cfsource, rwsched_CFRunLoopSourceRef);
  rwsched_tasklet_CFRunLoopAddSource(instance->rwtasklet_info->rwsched_tasklet_info, runloop, cfsource, instance->rwtasklet_info->rwsched_instance->main_cfrunloop_mode);
}
...
bool RwCLI::load_manifest_files()
{
  ...
  if (!fs::exists(cli_dir)) {
    std::cerr << "CLI manifest dir doesn't exists " << cli_dir << std::endl;
    return false;
  }
...
bool RwCLI::load_schema_modules(const char * schema_listing_filename)
{
  std::string const schema_listing =
      std::string(getenv("RIFT_INSTALL")) + std::string("/usr/data/manifest/") + schema_listing_filename;

  if (!fs::exists(schema_listing)) {
    std::cerr << "Schema listing file doesn't exist " << schema_listing << std::endl;
    return false;
  }

  std::ifstream file(schema_listing);
  std::string module_name;

  while(std::getline(file, module_name)) {
    
    module_names_.insert(module_name);
...
void RwCLI::print_element_names(LIBXML::xmlNode *node, int indent)
{
  LIBXML::xmlNode *cur_node = nullptr;
  int indent_increment = 0;

  for (cur_node = node; cur_node; cur_node = cur_node->next) {
    if (cur_node->type == LIBXML::XML_ELEMENT_NODE) {
      if ((strcmp((char *)cur_node->name, "message") != 0) &&
          (strcmp((char *)cur_node->name, "query") != 0)) {
        CLI_PRINTF("\n%*s%s: ", indent, " ", cur_node->name);
        indent_increment = 2;
      }
    } else if (cur_node->type == LIBXML::XML_TEXT_NODE) {
...
{
  LIBXML::xmlDoc *doc = nullptr;
  LIBXML::xmlNode *root_element = nullptr;
...
bool RwCLI::pretty_print_xml(char* xml_str, std::ostream& out)
{
  ...
  LIBXML::xmlChar *xml_buf = nullptr;
  int dump_size = 0;
...
void
xsdcli_component_init(void)
{
}

void
xsdcli_component_deinit(rwcli_component_ptr_t component)
{
  UNUSED(component);
}

void
xsdcli_instance_init(rwcli_component_ptr_t component,
                     rwcli_instance_ptr_t instance,
                     int argc,
                     char **argv)
{
  // ATTN: this method is not used anymore, RWCLI is always used
  // as a zsh plugin. Will be removed during the RwCLI cleanup
  rw_status_t status;
  ...
  rw_cli->component = component;
  rw_cli->instance = instance;

  bool ok = rw_cli->invoke_command_set(argc, argv);
  if (!ok) {
    ...
    RWTRACE_ERROR(instance->rwtasklet_info->rwtrace_instance,
                  ...
                  "Failed to parse initialization command in manifest.");
  }
}

void
xsdcli_instance_start(rwcli_component_ptr_t component,
		      rwcli_instance_ptr_t instance)
{
  UNUSED(component);

  //rw_cli->help_on_tab = true;
  RwCLI* rw_cli = static_cast<RwCLI*>(instance->rwcli);
  RW_ASSERT(rw_cli);

  rw_cli->readline_loop();
}
...
{
  RW_ASSERT(rwcli_zsh_instance);
  RW_ASSERT(rwcli_zsh_instance->schema_mgr_);
  RW_ASSERT(rwcli_zsh_instance->schema_mgr_->updater_);

  return rwcli_zsh_instance->schema_mgr_->updater_->get_handle();
}
...
void rwcli_schema_update_event()
{
  ...
  rwcli_zsh_instance->schema_mgr_->updater_->check_for_updates();
}
