...
static void launch_netconf_agent()
{
  ...
  switch(pid) {
    case 0: {
      ...
      snprintf(read_channel_fd_arg, 8, "%d", rwcli_agent_ch.in.fd.read);
      snprintf(write_channel_fd_arg, 8, "%d", rwcli_agent_ch.out.fd.write);
      ...
      for (fd = 3; fd < 65535; fd++) {
        if ((fd != rwcli_agent_ch.in.fd.read) &&
            (fd != rwcli_agent_ch.out.fd.write)) {
          close(fd);
        }
      }
