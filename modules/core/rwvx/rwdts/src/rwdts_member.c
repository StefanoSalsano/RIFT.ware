                               ...
                               rwdts_member_query_rsp_t*       rsp)
{
  ...
  if (match_info) {
    ...
    /* stop prep response timer */
    if (rsp->evtrsp != RWDTS_EVTRSP_ASYNC &&
        match_info->reg && match_info->reg->cb.cb.prepare &&
        match_info->prep_timer_cancel) {
      rwdts_member_prep_timer_cancel(match_info);
    }

    if (match_info->evtrsp != RWDTS_EVTRSP_ASYNC &&
        match_info->evtrsp != RWDTS_EVTRSP_NULL  &&
        match_info->reg && match_info->reg->cb.cb.prepare) {
      RW_ASSERT(rsp->evtrsp != RWDTS_EVTRSP_ASYNC);
    }
  }
...
rwdts_store_cache_obj(rwdts_xact_t* xact)
{
  ...
  HASH_ITER(hh_data, xact->obj_list, mobj, tmp) {
...
void rwdts_group_destroy(rwdts_group_t *grp) {
  ...
  RW_ASSERT(RW_DL_LENGTH(&grp->regs) == 0);
  RW_ASSERT(apih->group);
