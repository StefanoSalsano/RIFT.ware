...
#define RWDTS_ASYNC_TIMEOUT (60 * NSEC_PER_SEC)
#define PREPARE_CB_CANCEL_TIMER(match)  (!match->prep_timer_cancel)
  ...
  if ((reg->flags & RWDTS_FLAG_PUBLISHER) &&
    ...
    if(rw_keyspec_path_has_wildcards(trunc_ks)) {
      if (reg->shard && reg->shard->appdata) {
        ...
        rwdts_member_cursor_t *cursor = rwdts_member_data_get_current_cursor(xact,
