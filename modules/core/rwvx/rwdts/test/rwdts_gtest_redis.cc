...
static void memberapi_member_start_multi_xact_f(void *ctx) {
  ...
  if (s->member_startct == TASKLET_MEMBER_CT) {
    ...
    rwsched_dispatch_async_f(s->tenv->t[TASKLET_CLIENT].tasklet, s->client.rwq, &s->tenv->t[TASKLET_CLIENT], memberapi_client_start_f);
  }
...
TEST_F(RWDtsRedisEnsemble, InitRedis)
{
...
TEST_F(RWDtsRedisEnsemble, InitRedisXact)
{
  rwsched_dispatch_main_until(tenv.redis_tasklet, 10/*s*/, &tenv.redis_up);
