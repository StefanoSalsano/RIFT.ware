...
static void memberapi_reg_dereg_client_start_f(void *ctx) 
{
  ...
  rs = rwdts_member_reg_handle_create_element(regh,
                                              gkpe,
                                              &phone->base,
                                              NULL);
...
static int memberapi_get_objct(void *ctx) {
  ...
  if (reg) {
    ...
    rwdts_member_cursor_t *cur = rwdts_member_data_get_current_cursor(NULL, regh);
    rwdts_member_data_reset_cursor(NULL, regh);
...
static int memberapi_dumpmember_nc(struct tenv1::tenv_info *ti,
  ...
  if (reg) {
    ...
    ks=NULL;
    rwdts_member_cursor_t *cur = rwdts_member_data_get_current_cursor(xact, regh);
    rwdts_member_data_reset_cursor(xact, regh);
                                             ...
                                             const rwdts_commit_record_t** crec)
{
  ...
  rwdts_member_cursor_t* cursor = rwdts_member_data_get_current_cursor((rwdts_xact_t*)xact_info->xact, 
                                                                       s->regh[ti->tasklet_idx]);
