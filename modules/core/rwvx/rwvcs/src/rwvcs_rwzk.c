...
void rwvcs_rwzk_seed_auto_instance_sync_f(void *ctx)
{
  ...
  exists = rwcal_rwzk_exists(rwvcs->rwvx->rwcal_module, instance_path);
  if (!exists) {
    char * data = NULL;
...
void rwvcs_rwzk_lock_sync_f(void *ctx)
{
  ...
  rwvcs_rwzk_lock_sync_p->status = rwcal_rwzk_lock(rwvcs->rwvx->rwcal_module, path, timeout);
  free(path);
...
static void rwvcs_rwzk_unlock_sync_f(void *ctx)
{
  ...
  rwvcs_rwzk_unlock_sync_p->status = rwcal_rwzk_unlock(rwvcs->rwvx->rwcal_module, path);
  free(path);
...
void rwvcs_rwzk_node_exists_sync_f(void *ctx)
{
  ...
  rwvcs_rwzk_node_exists_sync_p->ret = rwcal_rwzk_exists(rwvcs->rwvx->rwcal_module, path);
  free(path);
    ...
    const char * instance_name)
{
  ...
  status = rwcal_rwzk_create(rwvcs->rwvx->rwcal_module, path, NULL);
  free(path);
    ...
    int * data_len)
{
  ...
  status = rwcal_rwzk_get(rwvcs->rwvx->rwcal_module, path, data, NULL);
  free(path);
...
static void rwvcs_rwzk_node_update_f(void *ud)
{
  ...
  status = rwcal_rwzk_lock(rwvcs->rwvx->rwcal_module, path, &timeout);
  ...
  status = rwcal_rwzk_set(rwvcs->rwvx->rwcal_module, path, c_string, NULL);

  unlock_status = rwcal_rwzk_unlock(rwvcs->rwvx->rwcal_module, path);
  ...
  if (rwvcs->config_ready_fn) {
    ...
    RWTRACE_INFO(
        rwvcs->rwvx->rwtrace,
        RWTRACE_CATEGORY_RWVCS,
        "Performing node update for %s:%d ",
        path, id->state);
  CFRelease(cfstring);
  RW_FREE(path);
...
static void rwvcs_rwzk_delete_node_sync_f(void *ctx)
{
  ...
  rwvcs_rwzk_delete_node_sync_p->status = rwcal_rwzk_delete(rwvcs->rwvx->rwcal_module, path, NULL);
  free(path);
  return;
}
...
static void rwvcs_rwzk_next_instance_id_sync_f(void *ctx)
{
  ...
  rwvcs_rwzk_next_instance_id_sync_p->status = rwcal_rwzk_lock(rwvcs->rwvx->rwcal_module, instance_path, &tv);
  RW_ASSERT(rwvcs_rwzk_next_instance_id_sync_p->status == RW_STATUS_SUCCESS);

  rwvcs_rwzk_next_instance_id_sync_p->status = rwcal_rwzk_get(rwvcs->rwvx->rwcal_module, instance_path, &data, NULL);
  RW_ASSERT(rwvcs_rwzk_next_instance_id_sync_p->status == RW_STATUS_SUCCESS);
  ...
  rwvcs_rwzk_next_instance_id_sync_p->status = rwcal_rwzk_set(rwvcs->rwvx->rwcal_module, instance_path, data, NULL);
  RW_ASSERT(rwvcs_rwzk_next_instance_id_sync_p->status == RW_STATUS_SUCCESS);

  rwvcs_rwzk_next_instance_id_sync_p->status = rwcal_rwzk_unlock(rwvcs->rwvx->rwcal_module, instance_path);
  RW_ASSERT(rwvcs_rwzk_next_instance_id_sync_p->status == RW_STATUS_SUCCESS);
  RW_FREE(data);
  return;
}
