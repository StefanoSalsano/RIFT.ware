...
class BaseCli
{
 ...
 public:
  ...
  /// Display help on TAB. ATTN: this belongs in EditLine?
  bool help_on_tab_;
...
/**
 * libedit front-end for the base CLI.
 */
class CliEditline
{
  public:
    CliEditline(BaseCli& cli, FILE* in, FILE* out, FILE* err);
    ~CliEditline();

    static const char *libedit_prompt_cb(EditLine *el);
    static unsigned char libedit_helpkey_cb(EditLine *el, int ch);
    static unsigned char libedit_tabkey_cb(EditLine *el, int ch);
    bool libedit_enterkey_cb(EditLine *el, bool interactive);
    void readline_loop();
    void quit();
    void nonblocking_mode();
    void read_ready();
    bool should_quit();

  public:
    /// The associated CLI instance
    BaseCli& cli_;

    /// The libedit instance
    EditLine *el;

    /// The libedit command line history
    History *hist;

    HistEvent ev;

    /// Flag for stopping the readline loop
    bool quit_readline_loop;
};
