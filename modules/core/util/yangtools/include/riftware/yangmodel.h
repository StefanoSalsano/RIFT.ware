...
public:
  ...
  virtual YangNode* search_node(
    ...
    const char *ns = nullptr //!< [in] The namspace.  May be nullptr.
  );
...
public:
   ...
   * node name and namspace.  If the namespace is not specified, then
  ...
  virtual YangNode* search_child_with_prefix(
    const char * name, //!< [in] Node name to search for.
    const char * prefix//!< [in] The namspace.
  );
  ...
  /*!
   * Search this node's children for a node that matches the specified
   * node name and namspace, if the nodes original name were mangled
   * according to rules used by YangToPB methods. The namespace is optional,
