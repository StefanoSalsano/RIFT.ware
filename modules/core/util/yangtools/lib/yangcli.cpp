...
void ParseLineResult::adjust_subtrees(ParseNode* parse_found)
{
  ...
  if (parse_node_->is_hide_key_keyword() &&
    ...
    result_node_ = result_found;
      ...
      parse_node_ != parse_found->parent_) {
    RW_ASSERT(parse_node_->parent_ == parse_found->parent_); // siblings
...
: model_(model),
  ...
  current_mode_(NULL),
  prompt_(),
  help_on_tab_(true), // ATTN: This really belongs in the EditLineCli object
...
void BaseCli::setprompt(void)
{
  ...
  // It will be picked up the editline prompt callback function
...
/*****************************************************************************/
/**
 * Create an editline CLI object and initialize it.
 */
CliEditline::CliEditline(BaseCli& cli, FILE* in, FILE* out, FILE* err)
   : cli_(cli)
{
  // Initialize editline
  // Set the client data handle to the class pointer
  el = el_init("libedit", in, out, err);
  el_set(el, EL_SIGNAL, 1);
  el_set(el, EL_CLIENTDATA, this);

  // Initialize the history library
  // Remember 100 history events
  // Use emacs command line processing
  hist = history_init();
  ...
  el_set(el, EL_PROMPT_ESC, libedit_prompt_cb, '\1');

  // Set the tabkey callback function
  el_set(el, EL_ADDFN, "ed-complete", "Complete", libedit_tabkey_cb);
  el_set(el, EL_BIND, "^I", "ed-complete", NULL);

  // Set the helpkey callback function
  el_set(el, EL_ADDFN, "ed-help", "Display help", libedit_helpkey_cb);
  el_set(el, EL_BIND, "?", "ed-help", NULL);
}

/**
 * Destroy an editline CLI.
 */
CliEditline::~CliEditline()
{
   history_end(hist);
   el_end(el);
}
...
/**
 * Handle the prompt callback from libedit.
 */
const char *CliEditline::libedit_prompt_cb(EditLine *el)
{
  CliEditline *editline;
  el_get(el, EL_CLIENTDATA, &editline);
  RW_ASSERT(editline);
  return editline->cli_.prompt_;
}

/**
 * Handle help key ('?') callback from libedit.
 */
unsigned char CliEditline::libedit_helpkey_cb(EditLine *el, int ch)
{
  (void)ch;

  // Get the current buffer from editline
  int line_buffer_size = el_line(el)->lastchar - el_line(el)->buffer;
  std::string line_buffer = std::string(el_line(el)->buffer, line_buffer_size);

  // Get the class handle to the class installed into editline instance
  CliEditline *editline;
  el_get(el, EL_CLIENTDATA, &editline);
  RW_ASSERT(editline);

  editline->cli_.generate_help(line_buffer);

  return CC_REDISPLAY;
}

/**
 * Handle complete key (TAB) callback from libedit.
 */
unsigned char CliEditline::libedit_tabkey_cb(EditLine *el, int ch)
{
  int res = CC_ERROR;
  (void)ch;
  ...
  std::string linepath = editline->cli_.tab_complete(line_buffer);
  if (linepath.length() == 0) {
    goto done;
  }

  // Update the editline line buffer
  el_deletestr(el, line_buffer_size);
  if (el_insertstr(el, linepath.c_str()) == -1) {
    res = CC_ERROR;
  } else {
    //res = CC_REFRESH;
    res = CC_REDISPLAY;
...
/**
 * Handle complete line form libedit.  This is not actually a callback;
 * the call comes from this class itself.
 */
bool CliEditline::libedit_enterkey_cb(EditLine *el, bool interactive)
{
  // Get the class handle to the class installed into editline instance
  CliEditline *editline;
  el_get(el, EL_CLIENTDATA, &editline);
  ...
  line_buffer_size = el_line(el)->lastchar - el_line(el)->buffer;
  line_buffer = std::string(el_line(el)->buffer, line_buffer_size);

  return cli_.parse_line_buffer(line_buffer, interactive);
}

/**
 * Quit the CLI.
 */
void CliEditline::quit()
{
  quit_readline_loop = true;
}

/**
 * Run the interactive CLI in blocking mode.
 */
void CliEditline::readline_loop()
{
  el_set(el, EL_UNBUFFERED, 0);
  quit_readline_loop = false;
  while (!quit_readline_loop) {
    int num;
    const char* buffer = el_gets(el, &num);
    libedit_enterkey_cb(el, true);
      // add to history
    if (strlen(buffer) && (strlen(buffer) != strspn(buffer, " \t\r\n"))) {
...
/**
 * Setup CLI to run in non-blocking mode.
 */
void CliEditline::nonblocking_mode()
{
 ...
 * Should quit the CLI?
 */
bool CliEditline::should_quit()
{
  return quit_readline_loop;
}
